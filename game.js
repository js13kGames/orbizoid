...
menuStyle = byId("menu").style;
cId.width = hud.width = ctxCopy.width = W;
cId.height = hud.height = ctxCopy.height = H;

for (s = 0; s < 18; s++) explosionSin.push(Math.sin(PI / 9.0 * s)), explosionCos.push(Math.cos(PI / 9.0 * s));

byId("easy").onclick = function () {
...
function getPlanets() {
    p = [
        { score: 100, die: false, e: 0.5, w: 15, o: PI * 1.4, d: 100, s: 0.00, x: cId.width / 2, y: cId.height / 2, c: 0, rx: 0, ry: 0 },
        { score: 200, die: false, e: 0.4, w: 10, o: PI, d: 70, s: 0.05, x: cId.width / 2, y: cId.height / 2, c: 56, rx: 0, ry: 0 },
        { score: 300, die: false, e: 0.3, w: 7, o: PI / 2, d: 80, s: 0.075, x: cId.width / 2, y: cId.height / 2, c: 118, rx: 0, ry: 0 },
        { score: 400, die: false, e: 0.2, w: 10, o: PI / 3, d: 40, s: 0.1, x: cId.width / 2, y: cId.height / 2, c: 240, rx: 0, ry: 0 },
        { score: 500, die: false, e: 0.05, w: 5, o: PI * 1.5, d: 150, s: 0.2, x: cId.width / 2, y: cId.height / 2, c: 290, rx: 0, ry: 0 }];
    for (var i = 0; p[i]; i++) {
...
function init() {
    ...
    game = {
        player: {
            ...
            y: cId.height / 2,
            w: 25, doubleScore: 0,
            c: "rgba(220,220,220,.9)",
            c2: "rgba(220,220,220,.6)",
            powerupDuration: 0,
    ...
    for (i = 0; i < numStars; i++) {
        game.starfield.stars.push({
            x: Math.random() * W * 10 - 5 * W,
            y: Math.random() * H * 10 - 5 * H,
            z: Math.random() * 100
        });
    }
    ...
    game.bonus.push({
        r: 0, x: 250, y: 250, c: cnv, p: 10, duration: 300, busy: 0,
    ...
    cnv.src = c.toDataURL("image/png");
    game.bonus.push({
        r: 0, x: 350, y: 250, c: cnv, p: 3, duration: 0, busy: 0,
        start: function () {
            if (game.player.level > 1) {
                game.player.level--;
                game.settings.numPlanets--;
            }
            game.player.progress = 0; invalidatedHud = 1;
    ...
    cnv.src = c.toDataURL("image/png");
    game.bonus.push({
        r: 0, x: 450, y: 250, c: cnv, p: 25, duration: 0, busy: 0,
        start: function () {
    ...
    cnv.src = c.toDataURL("image/png");
    game.bonus.push({
        r: 0, x: 550, y: 250, c: cnv, p: 2, duration: 0, busy: 0,
        start: function () { game.player.lives++; invalidatedHud = 1; },
        done: function () { }
    });
    ...
    m.lineWidth = 3;
    for (i = 0; i < 8; i++) {
        m.moveTo(spriteSize + Math.sin(d * i) * 4 + 16, spriteSize + Math.cos(d * i) * 4 + 16);
        m.lineTo(spriteSize + Math.sin(d * i) * 16 + 16, spriteSize + Math.cos(d * i) * 16 + 16);
    }
    m.stroke();
    var cnv = document.createElement("img");
    cnv.src = c.toDataURL("image/png");
    game.bonus.push({
        r: 0, x: 650, y: 250, c: cnv, p: 7, duration: 0, busy: 0,
        start: function () { for (i = 0; game.enemy[i]; i++) game.enemy[i].die = 1; },
        done: function () { }
    });
}
...
function renderStar(color) {
    ...
    for (p = 0; p < point * 2; p++) {
        m.lineTo(Math.sin(p * s) * ((p % 2) * 12 + 6) + spriteSize, Math.cos(p * s) * ((p % 2) * 12 + 6) + spriteSize);
    }
    m.closePath();
    m.fill();
    m.stroke();
...
function newEnemy() {
    var x = Math.random() * W,
        ...
        p = game.enemy.push({
            ...
            o: 0,
            s: Math.random() * 3 + 1,
            t: Math.random() * game.settings.numPlanets | 0
        }) - 1;
...
function recalculate() {
    ...
    for (var i = 0; i < len; i++) {
        ...
        game.planets[i].rx = ((0.5 + game.planets[i].x + Math.sin(game.planets[i].o) * game.planets[i].d) | 0);
        game.planets[i].ry = ((0.5 + game.planets[i].y + Math.cos(game.planets[i].o) * game.planets[i].d) | 0);
    ...
    for (i = 0; i < len; i++) {
        ...
        game.enemy[i].x += -Math.sin(game.enemy[i].d) * game.enemy[i].s;
        game.enemy[i].y += -Math.cos(game.enemy[i].d) * game.enemy[i].s;
            ...
            for (j = 0; j < game.settings.numPlanets; j++) {
                if (game.enemy[i].t == j) {
                    if (distance2D(game.enemy[i].x, game.enemy[i].y, game.planets[j].rx, game.planets[j].ry) < (20 + game.planets[j].w)) {
                        ...
                        var rnd = Math.random() + Math.random() * 250;
                        last = 0;
...
function draw() {
    ...
    if (ready) {
        ...
        for (j = 0; j < len; j++) {
            ...
            r = Math.cos(game.powerups[j].r) / 8 + Math.PI * .32;
            ctxBuffer.strokeStyle = "rgba(255,220,0,.5)";


            for (var i = 0; i < 2 * Math.PI; i += .1) {
                x = game.powerups[j].x + 16 - (8 * Math.sin(i)) * Math.sin(r * Math.PI) + (48 * Math.cos(i)) * Math.cos(r * Math.PI);
                y = game.powerups[j].y + 16 + (48 * Math.cos(i)) * Math.sin(r * Math.PI) + (8 * Math.sin(i)) * Math.cos(r * Math.PI);
                if (i == 0) {
        ...
        for (i = 0; i < len; i++) {
            ...
            for (a = 3; a >= 1; a--) {
                ...
                for (j = 0; j < 18; j++) {
                    ctxBuffer.moveTo(explosionSin[j] * (o1 + 3) * 25 + game.explosions[i].x,
                        explosionCos[j] * (o1 + 3) * 25 + game.explosions[i].y);
                    ctxBuffer.lineTo(explosionSin[j] * (o1) * 25 + game.explosions[i].x,
                        explosionCos[j] * (o1) * 25 + game.explosions[i].y);
                }
                ctxBuffer.stroke();
            }
        }
...
function updateHud() {
    var pc = document.createElement('canvas'), pm = pc.getContext("2d");
    ...
    var pat = ctxHud.createPattern(pi, "repeat");
    ...
    for (var i = 0; i < game.levels[game.player.level - 1].m * 2; i += 2) {
        ctxHud.moveTo(px + Math.sin(s * i) * 50, py + Math.cos(s * i) * 50);
        ctxHud.lineTo(px + Math.sin(s * i) * 50, py + Math.cos(s * i) * 50);
        ctxHud.lineTo(px + Math.sin(s * (i + 1)) * 50, py + Math.cos(s * (i + 1)) * 50);
        ctxHud.lineTo(px + Math.sin(s * (i + 1)) * 30, py + Math.cos(s * (i + 1)) * 30);
        ctxHud.lineTo(px + Math.sin(s * i) * 30, py + Math.cos(s * i) * 30);
        ctxHud.closePath();
    }
    ctxHud.stroke();
    ...
    ctxHud.beginPath();
    ctxHud.lineWidth = 2;
    //  px = py = 60; s = PI / game.levels[game.player.level - 1].m;
...
function countdown() {
    ...
    byId("#3").className += " countIn";
    setTimeout(function () {
        byId("#3").className += " countOut";
        byId("#2").className += " countIn";
        setTimeout(function () {
            byId("#2").className += " countOut";
            byId("#1").className += " countIn";
            setTimeout(function () {
                byId("#1").className += " countOut";
                setTimeout(function () {
                    byId("countdown").style.display = "none";
                    byId("#1").className = byId("#2").className = byId("#3").className = "count";
                    ready = true;
                }, 500);
            }, 1000);
        }, 1000);
